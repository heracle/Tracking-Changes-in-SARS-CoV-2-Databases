...
if [ -z ${2} ]; then lookup_output_merge_arg=""; else lookup_output_merge_arg="--lookup_output $(pwd)/${2}"; fi
...
bsub -W 4:00 -J "split_${bs_name}" -w "done(unxz_${bs_name})" -R "rusage[mem=280000]" python "/cluster/home/rmuntean/git/tracking-changes/scripts/split_snapshot.py" -i "${tmp_filepath}" -o "${tmp_dir}" -n "${cores}" --lookup_hash_align "/cluster/home/rmuntean/git/tracking-changes/data/lookup_2022-03-13.json.xz"
